...
from frame_timing_agent.ui.style import OPERATION_COLORS, OPERATION_LABELS, make_icon
...
class BatchWorkspace(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        create_actions = QFrame()
        create_actions.setObjectName("batchCreateActions")
        create_layout = QHBoxLayout(create_actions)
        create_layout.setContentsMargins(0, 0, 0, 0)
        create_layout.setSpacing(8)
        self.add_button = QPushButton("添加目录")
        self.add_button.setIcon(make_icon("add"))
        self.discover_button = QPushButton("扫描根目录")
        self.discover_button.setIcon(make_icon("discover"))
        self.start_button = QPushButton("开始批次")
        self.start_button.setObjectName("primaryButton")
        self.start_button.setIcon(make_icon("play", "#ffffff"))
        for button in (self.add_button, self.discover_button, self.start_button):
            create_layout.addWidget(button)
        create_layout.addStretch()
        self.summary_label = QLabel("尚未创建批次")
        self.summary_label.setObjectName("muted")
        create_layout.addWidget(self.summary_label)
        action_layout.addWidget(create_actions)

        run_actions = QFrame()
        run_actions.setObjectName("batchRunActions")
        run_layout = QHBoxLayout(run_actions)
        run_layout.setContentsMargins(0, 0, 0, 0)
        run_layout.setSpacing(8)
        self.pause_button = QPushButton("当前项完成后暂停")
        self.pause_button.setIcon(make_icon("pause"))
        self.continue_button = QPushButton("继续")
        self.continue_button.setIcon(make_icon("resume"))
        self.retry_button = QPushButton("重试选中失败项")
        self.retry_button.setIcon(make_icon("retry"))
        self.export_button = QPushButton("导出可用结果")
        self.export_button.setIcon(make_icon("output"))
        self.open_batch_button = QPushButton("打开批次产物")
        self.open_batch_button.setIcon(make_icon("open"))
        for button in (
            self.pause_button,
        ...
        run_layout.addWidget(self.open_batch_button)
        action_layout.addWidget(run_actions)
        ...
        analysis_title = QLabel("时序分析")
        analysis_title.setObjectName("batchAnalysisTitle")
        analysis_title.setProperty("role", "subsection")
        ...
        frames_title = QLabel("代表帧")
        frames_title.setObjectName("batchFramesTitle")
        frames_title.setProperty("role", "subsection")
        detail_layout.addWidget(frames_title)
        ...
        review_section.setObjectName("batchReviewSection")
        review_layout = QVBoxLayout(review_section)
        review_layout.setContentsMargins(0, 0, 0, 0)
        review_layout.setSpacing(6)
        review_title = QLabel("人工复核")
        review_title.setObjectName("batchReviewTitle")
        review_title.setProperty("role", "subsection")
        ...
        self.approve_button.setIcon(make_icon("check"))
        note_row.addWidget(self.note_edit, 1)
        note_row.addWidget(self.approve_button)
        review_layout.addLayout(note_row)

        output_section = QFrame()
        output_section.setObjectName("batchOutputSection")
        output_layout = QVBoxLayout(output_section)
        output_layout.setContentsMargins(0, 0, 0, 0)
        output_layout.setSpacing(6)
        output_title = QLabel("输出位置")
        output_title.setObjectName("batchOutputTitle")
        output_title.setProperty("role", "subsection")
        ...
        self.artifact_path.setFixedHeight(30)
        ...
        output_layout.addWidget(self.artifact_path)
        output_layout.addWidget(self.output_path)
        footer.addWidget(review_section, 1)
        footer.addWidget(output_section, 1)
        ...
        status_bar = QFrame()
        status_bar.setObjectName("batchStatusBar")
