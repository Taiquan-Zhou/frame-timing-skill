...
- Add persistent local settings, run history, frozen historical thumbnails, and managed output deletion.
- Add progress reporting, synchronized chart hover inspection, and representative-frame previews.
- Bind exports to the analyzed input and strategy digests, verify copied output bytes, and commit generated output atomically.
- Keep the Agent-safe v3 CLI and typed service lifecycle as the programmatic integration path.
- Publish a portable Windows x64 download alongside the Python package artifacts.
