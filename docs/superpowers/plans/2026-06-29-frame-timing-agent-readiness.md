...
|---|---:|---:|---|
| `coverage_first` | 0.85 | 2 | 只删除高置信度抖动、质量替代或高置信静止冗余帧，v3 Agent-safe CLI 默认使用 |
...
验收：0-136 和 424-579 不得判为静止；任何删帧都必须来自高置信度抖动、质量替代或高置信静止冗余稀疏判断，并满足覆盖约束。报告措辞必须写“当前验收集中观察到 0 个慢速运动静止误判”，不得写成对未知视频的零误判保证。
...
当前验收集内已确认的慢速运动误判静止数、视差/动态前景不确定区间自动删除数和验证器违规漏过数必须均为 0；`coverage_first` 只允许高置信静止冗余稀疏，不允许把 expected-active 范围判为静止后删除；所有高风险策略必须要求人工确认。算法效果不满足门槛时不发布 v0.3.0，也不得修改期望结果来迁就实现。

通过这些门槛只允许发布独立的 v3 Agent-safe API/CLI，不自动授权迁移 legacy facade 或 `frame-timing` 默认入口。旧入口迁移需基于本次 benchmark 结果另立计划和版本。

2026-07-04 当前业务样本验收：6 个原始视频切分为 7 个真实案例，另加 1 个 60 帧纯静止基线；`coverage_first` 的完整生命周期均通过机器校验，人工确认默认实际输出连续，静止与抖动判断在当前样本范围内正确。`balanced` 只作人工比较，`jitter_reduction` 在前向视差和部分低清晰度运动段产生明显跳跃，因此不得自动执行。完整建模覆盖风险仍需后续处理模块共同验证。

通用开源发布门仍保持 pending：当前批次不覆盖手持抖动、快速主动转向和独立运动前景，且 benchmark schema 尚不能表达“默认策略通过、比较策略失败”的逐策略人工结论。不得把当前业务范围通过扩大为未知视频或完整重建链路保证。
